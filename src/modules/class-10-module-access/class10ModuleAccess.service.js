...
export const CLASS_10_SUPPORTED_CLASSES = [10];
export const CLASS_10_MODES = ["basic", "professional"];
...
function normalizeClassNumber(className) {
  ...
  if (upper === "X" || upper === "CLASS X" || upper === "STD X") return 10;
  return null;
}
...
function buildAvailability(eligible) {
  return {
    supported_classes: CLASS_10_SUPPORTED_CLASSES,
    supported_modes: CLASS_10_MODES,
    reason: eligible
      ? "Eligible for this module."
      : "This module is currently available only for class 10 students.",
  };
}

export async function buildClass10ModuleResponse({
  user,
  query,
  mode = "basic",
  moduleDefinition,
}) {
  const selectedMode = CLASS_10_MODES.includes(mode) ? mode : "basic";
  ...
  const eligible = CLASS_10_SUPPORTED_CLASSES.includes(classNumber);
