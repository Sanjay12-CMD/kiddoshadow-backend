...
export const CLASS_89_SUPPORTED_CLASSES = [8, 9];
export const CLASS_89_MODES = ["basic", "professional"];
...
function normalizeClassNumber(className) {
  ...
  if (upper === "VIII" || upper === "CLASS VIII" || upper === "STD VIII") return 8;
  if (upper === "IX" || upper === "CLASS IX" || upper === "STD IX") return 9;
  return null;
}
...
function buildAvailability(eligible) {
  return {
    supported_classes: CLASS_89_SUPPORTED_CLASSES,
    supported_modes: CLASS_89_MODES,
    reason: eligible
      ? "Eligible for this module."
      : "This module is currently available only for class 8 and class 9 students.",
  };
}

export async function buildClass89ModuleResponse({
  user,
  query,
  mode = "basic",
  moduleDefinition,
}) {
  const selectedMode = CLASS_89_MODES.includes(mode) ? mode : "basic";
  ...
  const eligible = CLASS_89_SUPPORTED_CLASSES.includes(classNumber);
